...
# Automatically sets up a worker node in a K8s cluster to be used on SLATE (CentOS 7)
# Worker node will still need to be joined to cluster with the join
# command generated by `kubeadm token create --print-join-command`
# 
